...
def _get_datasets(
    ...
    # Check if it is an evaluation, if so, use the entire data.
    if is_evaluation:
...
# Any function that has the context of `mlrun.MLClientCtx` can become an MLRun handler.
def train(
    ...
    mlrun_tf_keras.apply_mlrun(
        model=model, model_name="mask_detector", context=context)
