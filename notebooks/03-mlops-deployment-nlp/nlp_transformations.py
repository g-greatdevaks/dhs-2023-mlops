...
class SummarizeArticle:
    ...
    def do(self, event):
        event["summarized_text"] = self.summarizer(
            event["original_text"], truncation=True)[0]['summary_text']
        return event
...
def kv_format(event):
    """
    Format record to be stored in Redis KV table. Removes apostrophes in text fields,
